...
public class ShapelessCraftingGUI extends CraftingTableGui {

    private final HumanEntity viewer;

    public ShapelessCraftingGUI(CustomRecipe r, ListAllRecipesGUI parent, HumanEntity entity) {
        super(r.getName(), CustomRecipes.getInstance());
        this.viewer = entity;

        ShapelessRecipe recipe = ((ShapelessRecipe) r.getRecipe());
        ...
        for (ItemStack stack : recipe.getIngredientList()) {
            input.addItem(new GuiItem(stack), i % 3, i / 3);
            i++;
        }
